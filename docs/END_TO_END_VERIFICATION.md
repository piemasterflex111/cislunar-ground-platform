# End-to-End Verification — Secure Payload Telemetry Gateway
...
- An end-to-end verification path covering nominal data, exact duplicates, and deliberately corrupted CRC data.
...
The command starts an isolated Compose project on local ports 8180 and 8181, so it does not reuse or disturb another running development stack. It verifies:
...
```bash
VERIFY_DEMO_KEEP_RUNNING=1 make verify-demo
```
