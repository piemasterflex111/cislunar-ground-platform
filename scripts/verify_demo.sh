...
GROUND_API_HOST_PORT="${VERIFY_DEMO_GROUND_API_HOST_PORT:-8180}"
VENDOR_SIMULATOR_HOST_PORT="${VERIFY_DEMO_VENDOR_SIMULATOR_HOST_PORT:-8181}"
...
KEEP_RUNNING="${VERIFY_DEMO_KEEP_RUNNING:-0}"
PROJECT_NAME="${VERIFY_DEMO_PROJECT_NAME:-cislunar-verification}"
...
heading "Secure Payload Telemetry Gateway — end-to-end verification"
echo "Verification path: raw preservation -> validation -> durable state -> queue -> worker result"
...
echo "PASS: end-to-end verification completed"

if [[ "$KEEP_RUNNING" == "1" ]]; then
  echo "Services remain running because VERIFY_DEMO_KEEP_RUNNING=1."
else
